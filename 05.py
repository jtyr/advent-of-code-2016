...
def main():
    ...
    while len(password1) < 8 or None in password2:
        checksum = hashlib.md5(
            (u"%s%d" % (door_id, idx)).encode('utf-8')).hexdigest()

        if checksum[:5] == '00000':
            if len(password1) < 8:
                password1 += checksum[5]
                print("p1: idx=%d; ch=%s" % (idx, checksum[5]))

            if None in password2:
                if (
                        checksum[5] in positions and
                        password2[int(checksum[5])] is None):
                    password2[int(checksum[5])] = checksum[6]
                    print("p2: idx=%d; ch=%s" % (idx, checksum[6]))

        idx += 1

    print("[Star 1] Password: %s" % password1)
    print("[Star 2] Password: %s" % ''.join(password2))
